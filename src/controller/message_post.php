...
try {
  ...
  $req->bindValue(':id_sujet', $id_sujet, PDO::PARAM_STR);
  $req->bindValue(':id_utilisateur', $id_utilisateur, PDO::PARAM_STR);
  $req->bindValue(':message', $message, PDO::PARAM_STR);
  $req->execute();
  $req->closeCursor();
  header("Location:../view/autre/Forum_sujet.php?id_sujet=".$id_sujet."&success=message_post#last_message"); //message posté
  exit;
}
catch(Exception $e)
{
  header('Location:../view/autre/Forum_sujet.php?id_sujet='.$id_sujet.'&error=2.1'); //echec de l'insertion des données
  exit;
}
