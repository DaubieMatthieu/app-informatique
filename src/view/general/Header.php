	...
	<header>
		<ul id="deroulant">
			...
			if (isset($_SESSION['role'])) //si l'utilisateur est connecté et qu'il a un role
			{
				...
				if ($role=="Administrateur") {$accueil="Accueil_admin.php";}
				if ($role=="Gestionnaire") {$accueil="Accueil_gestionnaire.php";}
				if ($role=="Utilisateur") {$accueil="Accueil_utilisateur.php";}
				...
				if(basename($_SERVER['PHP_SELF']) !==$accueil) {echo "<li><a href='../accueil/".$accueil."'>Accueil</a></li>";}
				if(basename($_SERVER['PHP_SELF']) !=='Profil.php') {echo '<li><a href="../autre/Profil.php">Profil</a></li>';}
