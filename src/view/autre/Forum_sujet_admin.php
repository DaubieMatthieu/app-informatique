...
<html>
    ...
    <section>
			...
			try {
				if (!isset($_GET['id_sujet'])) {header('Location:../autre/Forum_admin.php?error=6');exit;}
          $id_sujet=$_GET['id_sujet'];
					$req = "SELECT titre FROM sujet_forum WHERE id_sujet=$id_sujet";
				  $rep = $db->query($req);
					if($rep->rowCount() == 0) {header('Location:../autre/Forum_admin.php?error=6');exit;}
          $titre=$rep->fetch();
              ...
              try {
                ...
                $user=$rep3->fetch();
								...
								if ($role=='error') {
									...
									?>
									<br>
									<div class="utilisateur"><a href=<?php echo '../accueil/Accueil_admin?search='.$user['prenom']."+".$user['nom'];?>><?php echo $user['prenom']." ".$user['nom'].'<br>'.char_to_str($user['role']);?></a></div>
